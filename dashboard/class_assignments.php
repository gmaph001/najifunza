<?php
    ...
    if($result2){
        for($i=0; $i<mysqli_num_rows($result2); $i++){
            ...
            if($class === $row['class'] && $id === $row['poster']){
                $key[$size] = $row['assign_key'];
                $assign_name[$size] = $row['assign_name'];
                $assignment[$size] = $row['assignment'];
                $due[$size] = $row['due_date'];
                $submission[$size] = $row['submission'];
                $format[$size] = $row['format'];
                $size++;
            }
        }
    }
...
<body>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle notestitle">
            <h1>Assignments</h1>
            <?php echo "<a href='add-class-assignment.php?id=$id&&class=$class' class='add2'>New Assignment</a>";?>
        </div><!-- End Page Title -->
                ...
                if($size == 0){
                    ...
                    "
                        <div class='unavailable'>
                            <p>Sorry! You have not posted any assignment yet!</p>
                        </div>
                    ";
                }
                else{
                        ...
                        "
                            <div class='announcement'>
                                <div class='announce-top'>
                                    ...
                                    <span class='date'><b>$assign_name[$i]</b> <br> Due: $due[$i]</span>
                                    <div class='announce-btns'>
                                        <a class='view' onclick="; echo "bottomview('btm$n')"; echo " id='onbtm$n' name='0'>View</a>
                                        <a href='edit_class_assignment.php?id=$id&&class=$class&&key=$key[$i]' class='edit'>Edit</a>
                                        <a href='delete_class_assignment.php?id=$id&&class=$class&&key=$key[$i]' class='delete'>Delete</a>
                                    </div>
                                ...
                                if($submission[$i] === "yes"){
                                    ...
                                    "
                                        <div class='announce-bottom' id='btm$n'>
                                            <p class='announce-main'>
                                                This assignment requires submission of $format[$i] formats!
                                            </p>
                                            <div class='announce-view'>
                                                <span class='viewed'>Submission: </span>
                                                <a href='class_submissions_admin.php?id=$id&&class=$class&&key=$key[$i]' class='views'>Submission Box</a>
                                            </div><br>
                                            <a href='$assignment[$i]' class='view'>View assignment</a>
                                        </div>
                                    ";
                                }
                                else{
                                    ...
                                    "
                                        <div class='announce-bottom' id='btm$n'>
                                            <p class='announce-main'>
                                                This assignment requires no submission on the platform!
                                            </p>
                                            <a href='$assignment[$i]' class='view'>View assignment</a>
                                        </div>
                                    ";
    ...
    <?php echo "<a href='add-class-assignment.php?id=$id&&class=$class' class='add-notes'><span class='up-icon'>+</span><span class='up'>Upload</span></a>";?>
