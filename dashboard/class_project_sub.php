<?php
    ...
    $query3 = "SELECT * FROM class_project_submissions";
    ...
    if($result3){
        for($i=0;$i<mysqli_num_rows($result3); $i++){
            ...
            if($class === $row['class'] && $id === $row['submitter'] && $key === $row['proj_key']){
                $submission[$size] = $row['link'];
                ...
                if($result5){
                    for($j=0; $j<mysqli_num_rows($result5); $j++){
                        ...
                        if($sub_key[$size] === $row2['sub_key'] && $key === $row2['assign_key'] && $class === $row2['class']){
...
<body>
                    ...
                    if($size == 0){
                        ...
                        if($format === "document" || $format === "photo"){
                            ...
                            "
                                <div class='doc'>
                                    <img src='../media/icons/$format.png' class='docPic'>
                                    <a href='class_project_submissions2.php?id=$id&&class=$class&&key=$key&&sub=$sub_key[$n]' class='download'>Add File(s)</a><br>
                                    <a href='../$submission[$n]' target='_blank' class='preview'>View</a>
                                    <a class='preview' onclick=";?> "linkcpy('<?php echo "localhost/najifunza/ad.php?key=$sub_key[$n]";?> ')" <?php echo ">Copy Link</a>
                                </div>
                            ";
                        }
    ...
    <p class="linkcpymsg" id="lkmsg">Link copied successfully</p>
    ...
    <?php
        if($done !== "due"){
            ...
            if($size == 0){
                echo 
                "
                    <a href='class_project_submissions2.php?id=$id&&class=$class&&key=$key' class='new'>
                        <p class='plus'>+</p>
                ...
                echo 
                "
                    <a href='class_project_submissions2.php?id=$id&&class=$class&&key=$key&&sub=$sub_key[$n]' class='new'>
                        <p class='plus'>+</p>
                        <p class='newSub'>New Submissions</p>
                    </a>     
                ";
...
<script>
    ...
    function linkcpy(a){
        console.log(a);
        let msg = document.getElementById("lkmsg");

        navigator.clipboard.writeText(a);

        msg.setAttribute("class", "linkmsgdis");

        setTimeout(() => {
            msg.setAttribute("class", "linkmsg");
        }, 6000);
